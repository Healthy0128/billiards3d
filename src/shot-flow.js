...
function setCancelVisual(active){
  pullCancelled=active;
  handle.style.background=active?'rgba(180,45,45,.86)':'rgba(18,124,92,.82)';
  line.style.background=active?'rgba(255,110,110,.78)':'rgba(255,255,255,.72)';
  badge.style.background=active?'rgba(145,25,25,.9)':'rgba(0,0,0,.72)';
...
canvas?.addEventListener('pointerdown',e=>{
  ...
  pulling=true;pullCancelled=false;pullPointer=e.pointerId;pullStart={x:e.clientX,y:e.clientY};lastPull=null;pullPower=0;
  canvas.setPointerCapture(e.pointerId);showPull(e.clientX,e.clientY);setPhase('pull');e.preventDefault();
},{capture:true});
...
function finishSlingshot(e){
  ...
  if(pullCancelled||!lastPull||lastPull.dy<-CANCEL_MARGIN||lastPull.d<18||pullPower<4){
    pullPower=0;pullCancelled=false;lastPull=null;setPhase('aim');statusEl.textContent='ショットをキャンセルしました';return;
  }
  ...
  setTimeout(()=>{cameraBrowse();setPhase('view');pullPower=0;pullCancelled=false;lastPull=null},420);
}
...
for(const btn of [modeBtn,resetBtn])btn?.addEventListener('click',()=>setTimeout(()=>{pulling=false;pullCancelled=false;hidePull();cameraBrowse();setPhase('view')},0));
