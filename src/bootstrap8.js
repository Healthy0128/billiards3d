...
function installCueBallMarker(rt){
  ...
  g.fillStyle='rgba(255,210,70,.92)';g.fill();
  g.lineWidth=5;g.strokeStyle='rgba(20,15,5,.72)';g.stroke();
  ...
  const tick=()=>{
    ...
    if(cb&&!cb.pocketed&&!cb.falling){
      const p=cb.mesh?.position||cb.body?.position;
      if(p){marker.position.set(p.x,p.y+.155,p.z);marker.visible=true;}
...
async function loadRealTable(){
  ...
  try{
    ...
    requestAnimationFrame(()=>{if(rt.realTable?.parent){useRealModelWithInvisiblePhysics(rt);rt.statusEl.textContent=info.feltFound?'実モデルをそのまま表示 — プレイ用照明・透明物理壁':'実モデルをそのまま表示 — プレイ用照明・透明物理壁'}});
