...
async function bootGame(){
  ...
  source=source.replace("const table={w:2.84,h:1.42,y:.78,rail:.115},ballR=.05715,pocketR=.098,balls=[],rails=[];","const table={w:2.84,h:1.42,y:.78,rail:.115},ballR=.05715,pocketR=.098,balls=[],rails=[];window.__billiardsRuntime={scene,THREE,table,statusEl};");
  ...
  source=source.replace("let last=performance.now(),acc=0,wasMoving=false;const fixed=1/120;",`function applyRollingResistance(b,dt){\n  const v=b.body.velocity;const s=Math.hypot(v.x,v.z);\n  if(s<0.055){v.x=0;v.z=0;if(Math.abs(v.y)<0.03)v.y=0;const w=b.body.angularVelocity;if(w.length()<1.15)w.setZero();return;}\n  const decel=s>1.6?0.15:s>0.8?0.20:s>0.35?0.28:s>0.14?0.42:0.62;\n  const next=Math.max(0,s-decel*dt),k=next/s;v.x*=k;v.z*=k;\n  const spinKeep=Math.max(0,1-(s<0.22?1.8:0.5)*dt);b.body.angularVelocity.x*=spinKeep;b.body.angularVelocity.z*=spinKeep;\n  if(next<0.055){v.x=0;v.z=0;}if(Math.abs(v.y)<0.025)v.y=0;\n}\nlet last=performance.now(),acc=0,wasMoving=false;const fixed=1/120;`);
